    ...
    def set_lam_input(self, bounds, start):
        ...
        if (self.lam_temp is not None) and (self.lam is not None):
            ...
            if bounds is None:
                ...
                vlim = np.array([np.max(vlim) + 500, np.min(vlim) - 500])
...
################################################################################
    ...
    def nonlinear_fit(self, start0, bounds0, fixed0, tied0, clean):
        ...
        if self.global_search:
            glob_options = {'tol': 0.1, 'disp': 0}  # default
...
################################################################################
    ...
    def plot(self, color=['black', 'LimeGreen', 'blue', 'red'], gas_clip=False, spec=True, phot=None, lam_flam=False):
        """
        Produces a plot of the pPXF best fit.
        One can independently call ``pp.plot()`` after pPXF terminates.

        Use the command ``pp.plot(color=['black', 'LimeGreen', 'blue', 'red'])``
        a list of color names or hex values used to define the plot’s color scheme. 
        These colors applies as [spectrum, residuals, clipped spectrum, model]. 
        You can modify this list to customize the plot appearance.
        ...
        if self.lam is None:
            ...
            plt.xlabel(r"$\lambda_{\rm rest}$ ($Ångstrøm$)") #($\mu{\rm m}$)")
            x = self.lam #/scale
            if phot:
        ...
        if spec:
            ...
            ref = np.min(bestfit_shown) - 3*sig3
            ...
            plt.plot(x, galaxy, color[0], linewidth=1.2)
            plt.plot(x[self.goodpixels], resid[self.goodpixels], '+',
                     color=color[1], mec=color[1], ms=4)
            w = np.flatnonzero(np.diff(self.goodpixels) > 1)
            for wj in w:
                a, b = self.goodpixels[wj : wj + 2]
                plt.axvspan(x[a], x[b], facecolor='darkgray')
                plt.plot(x[a : b + 1], resid[a : b + 1], color[2], linewidth=1)
            for k in self.goodpixels[[0, -1]]:
                plt.plot(x[[k, k]], [ref, bestfit[k]], 'darkgray', linewidth=1)

            if self.gas_any:
                plt.plot(x, gas_bestfit_templates + ref, color[2], linewidth=1)
            ...
            plt.plot(x, stars_bestfit, color[3], linewidth=1)
        ...
        if phot:
            ...
            if spec:
                ...
                plt.axvspan(*x[[0, -1]], facecolor='darkgray', zorder=0)
                ll, rr, mn, mx = x1, x2, y1, y2s
