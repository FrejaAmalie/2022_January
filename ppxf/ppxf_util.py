...
Modified by [Freja] ([freja.noerby@nbi.ku.dk]) 
...
def determine_mask(ln_lam, lam_range_temp, redshift=0, width=800, custom_lines=None):
    """
    Modified by [Freja] ([freja.noerby@nbi.ku.dk]) to support custom emission
    lines and expanded number of defaul lines.
    ...
    :param ln_lam: Natural logarithm np.log(wave) of the wavelength in Angstrom
        of each pixel of the log rebinned *galaxy* spectrum.
    :param lam_range_temp: Two elements vectors [lam_min_temp, lam_max_temp]
        with the minimum and maximum wavelength in Angstrom in the stellar 
        *template* used in pPXF.
    :param redshift: Estimate of the galaxy redshift (default is 0).
    :param width: Width (in km/s) of the gas emission lines to mask (default is 800 km/s).
    :param custom_lines: Optional array of specific lines to mask. If None, the default 
        `lines` array will be used.
        
    :return: Boolean vector mask to be used as input for pPXF. Mask = True for pixels to include.
    """
    
    # Default list of emission lines (if no custom_lines provided)
    if custom_lines is None:
        #                  -----[OII]-----    Ne III   He I   [Ne III]   He,      [S II], [S II],   Hdelta   Hgamma  [O III],  He II,   Hbeta   [Fe VII]  -----[OIII]----- [Fe VII]  [Fe VII]    [OI]     [OI]    -----[NII]-----   Halpha   -----[SII]-----     He I  
        lines = np.array([3726.03, 3728.82, 3868.75, 3888.65, 3967.47, 3970.07, 4068.60, 4076.35, 4101.76, 4340.47, 4363.21, 4685.65, 4861.33, 4893.370, 4958.92, 5006.84, 5720.700, 6087.000, 6300.30, 6363.776, 6548.03, 6583.41, 6562.80, 6716.47, 6730.85, 7065.196])
    else:
        lines = custom_lines  # Use the custom array of lines if provided

    # Set the width of the masked region for each line (in km/s)
    dv = np.full_like(lines, width)  # width/2 of masked gas emission region in km/s
    c = 299792.458  # speed of light in km/s

    # Initialize the flag (which will track which pixels to mask)
    flag = False

    # Loop over each emission line and apply the mask based on the given width
    ...
    # Return the inverted flag as the mask (True for valid pixels, False for masked pixels)
    return ~flag
...
def mag_spectrum(lam, spectrum, bands='SDSS/r', redshift=0, system='AB', quiet=True):
    ...
    """
    p2 = synthetic_photometry(lam, spectrum, bands,
                              redshift=redshift, quiet=quiet)
...
def varsmooth(x, y, sig_x, xout=None, oversample=1):
    ...
    if np.isscalar(sig_x):
        ...
        assert np.all(np.isclose(
            dx[0], dx)), "`x` must be uniformly spaced, when `sig_x` is a scalar"
        n = len(x)
